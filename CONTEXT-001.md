...
● **The same cap applies to `/udl/elset`, and PSIRENS was breaking the rule.** FACT, 24 September 2026: until 1.6.6 the scheduled refresh asked for the whole retention window (90 days) of EVERY object in one request, with no slicing and no result cap. The response came back truncated, the newest fixes were the ones dropped, and the plot sat a CONSTANT ~14 days behind rather than frozen, because the window start advanced with the clock. Diagnosed from an inspector panel reading "Data age 14.1d" against an epoch of 2026-09-09. The slicing rule recorded above for the history endpoint is not endpoint-specific: apply it to every list query.
● INFERENCE, worth knowing before the next integration: a truncated page is INDISTINGUISHABLE from a complete one. Treat a response at or near any plausible cap as truncated and subdivide the window, rather than believing it. PSIRENS does this from 1.6.6 (`UDL_TRUNCATION_THRESHOLD`, default 9000).
● **A swallowed upstream failure reported no error, so a rejected feed looked identical to a quiet one.** FACT, 26 September 2026: a live `/api/meta` showed `stale: true`, `data_age_hours: 403.59`, `newest_sample_epoch` still 2026-09-09 (unmoved for three days), `last_ingest_added: 0`, `last_ingest_errors: []` and `last_successful_ingest: null`. The empty error list was not evidence of a healthy pull: `UDLElsetSource._request` catches every `httpx.HTTPError` so one bad slice never kills a cycle, logs a warning and returns None, and nothing counted it. 401, 403, 500 and a timeout all arrived at the store as "added 0, no errors". RULE, general: if a layer deliberately swallows a failure to stay resilient, that layer MUST count what it swallowed and surface it, or the resilience buys silence. PSIRENS counts requests, failures with reason, raw rows, distinct objects offered and objects kept after filtering from 1.6.9.
● INFERENCE, and the reason the counts are per stage: `added: 0` has at least four causes that need different actions - no request completed, the upstream rejected every request, the upstream answered with nothing for the window, or it answered with plenty that a client-side filter (here the high-interest list) then dropped. One number cannot separate them; `requests`/`failures`/`fetched`/`kept` can.
...
### App Store pipeline shape (corrected against a full pass, 2 September 2026)

● The pipeline is TEN stages, not nine: Secret Detection, Dependencies, SAST Scan, Dependency Scanning, Test, Code Quality, Dockerfile Lint, Container Build, Container Scan, then Deploy. The first nine appear under `Gates`; `Deploy` sits under a separate `Deployment` heading and the UI totals them as "All 10 stages passed".
● The tenth stage is invisible on a failed run. PSIRENS 1.5.0 and 1.5.2 both failed at Code Quality and showed only nine, which is how the register came to record nine. Never infer a pipeline's shape from failed runs alone; it understates it.
● The gates are NOT fail-fast: a Code Quality failure still lets Dockerfile Lint, Container Build and Container Scan run and report. Deploy is the exception and does not run at all, so a single failed gate blocks the release while the rest keep reporting green.
● PSIRENS 1.5.3 is the first build to pass all ten and reach Active. It is also the first to clear Code Quality, after 1.5.0 was rejected on `python:S107` and 1.5.2 on the throw-versus-rejected-promise rule. The local gates in `simulate-pipeline.sh`, each added after the failure that taught it, are what closed the gap.

### Object naming (PSIRENS, 2 September 2026)

● A UDL elset record's name is taken from `origObjectId`. This tenant leaves that field empty, so the stored name falls back to `satNo` and every REAL object ends up named after its own catalogue number. Any list that trusts the stored name renders "63157 63157".
● The JCO HRR list is the naming authority: it carries `commonName`. Resolve display names from it, falling back to a stored name ONLY when that name is not simply the id, then to the id alone. Never let the id stand in as a name inside the lookup map, because it will shadow a genuine stored name for a manual or demo object.
● The HRR cache is read back verbatim, and the bundled snapshot uses `name` while a raw JCO record uses `commonName`. Normalise on load; do not teach each consumer both keys. The SPA only ever knew `name`, so a cache in the other shape silently stripped names from the plot, the watchlist and the inspector.
...
● "Prefer `throw error` over `return Promise.reject(error)`" (inside a `then` callback) failed a real PSIRENS upload (1.5.2). MEASURED, not assumed: `eslint-plugin-sonarjs` does not carry this rule even with all 279 of its rules enabled, so running the local eslint proxy would NOT have caught it. This is the clearest evidence yet that the proxy cannot substitute for the server: only a real `sonar-scanner` run, or a rule-specific grep, will find this class.
● A verification step that cannot fail is worse than no step. An eslint run whose working directory does not contain the target file reports "File ignored because outside of base path" and exits 0, which reads as a pass. Every gate added to `simulate-pipeline.sh` is now proven to bite before being trusted: the complexity/parameter gate was tested against a 14-parameter probe, the SPA lint carries a permanent canary, and the grep gates were tested by reintroducing the banned pattern.
