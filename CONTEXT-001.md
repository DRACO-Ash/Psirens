...
● `simulate-pipeline.sh` mirrors the platform test stage (`pip install -r requirements.txt`, then pytest with an 80% coverage floor) and then runs reactive grep gates that pre-empt the rules above: a Python cognitive-complexity gate (the `cognitive_complexity` PyPI package matches the SonarSource algorithm exactly and fails any function over 15), `grep window.`, `grep role="(region|banner|navigation|main|contentinfo|complementary|form)"`, and `grep getAttribute("data-`.
