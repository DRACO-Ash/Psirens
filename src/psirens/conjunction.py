...
Both are built on the same validated Vallado SGP4 (`sgp4`) already used for
sub-satellite longitude, so nothing here is a hand-rolled propagator or a
hand-formatted TLE. TLE lines come from `sgp4.exporter.export_tle`, which owns
checksum and Alpha-5 catalogue-number handling; closest approach is a coarse
time scan refined around the minimum, computing the 3D TEME range between two
propagated states.

Inputs are the stored `elset` dicts (latest mean elements per object). An object
without an elset is simply skipped: it cannot be propagated, and this module
...
def tle_lines(el: dict) -> tuple[str, str] | None:
    ...
    Note: intl designator and rev number are not carried by the UDL elset and
    export blank/zero; the orbital fields and checksums are exact."""
    try:
...
def conjunctions_for(store_data: dict, target_id: str, *,
    ...
    return {
        "target": {"id": target_id, "name": target.get("name", target_id),
                   "tle": list(t_tle) if t_tle else None},
